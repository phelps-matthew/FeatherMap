...
def get_block_rows(i1: int, j1: int, i2: int, j2: int, n: int) -> List[int]:
    """Return range of full (complete) rows"""
    row = []
    # Handle all cases for j1=0
    if j1 == 0:
        # All row(s) complete
        if j2 == n - 1:
            row.extend(set([i1, i2]))
            return sorted(row)
        # First row complete, last row incomplete
        elif i2 > i1:
            row.extend(set([i1, i2 - 1]))
            return sorted(row)
...
print(get_block_rows(i1=42, j1=0, i2=42, j2=n - 1, n=n) == [42])
# All two rows complete
print(get_block_rows(i1=652, j1=0, i2=653, j2=n - 1, n=n) == [652, 653])
# All 10 rows complete
print(get_block_rows(i1=652, j1=0, i2=662, j2=n - 1, n=n) == [652, 662])
# First row complete, second incomplete
print(get_block_rows(i1=0, j1=0, i2=1, j2=n - 100, n=n) == [0])
# First row compete, last row incomplete
print(get_block_rows(i1=652, j1=0, i2=829, j2=105, n=n) == [652, 828])
# First row incomplete (from right), no additional rows
...
print(get_block_rows(i1=652, j1=1816, i2=829, j2=105, n=n) == [653, 828])
# First row incomplete (from left), second row complete
print(get_block_rows(i1=3000, j1=1816, i2=3001, j2=n-1, n=n) == [3001])
# First row incomplete (from left), last row complete
print(get_block_rows(i1=3000, j1=1816, i2=3091, j2=n-1, n=n) == [3001, 3091])
